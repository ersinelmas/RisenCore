...
function HabitItem({ habit, onUpdate, onDelete }) {
  ...
  const completedCount = useMemo(() => {
    if (isDaily) {
      ...
      return habit.completionDates.filter(date => currentWeekStrings.includes(date)).length;
    }
  ...
  const handleToggle = useCallback(async (date) => {
    ...
    if (isTogglingOn) {
      if (isDaily) {
        setTimeout(() => toast.success(`🎉 Great job on "${habit.name}" today!`), 300);
      } else {
        const newWeeklyCount = Array.from(newCompletions).filter(d => weekDays.map(formatDate).includes(d)).length;
        if (newWeeklyCount === habit.targetCount) {
          setTimeout(() => toast.success(`🎉 Awesome! You've hit your weekly goal for "${habit.name}"!`), 300);
        }
      }
    }
        ...
        <div className={styles.habitHeader}>
          <div>
            ...
            <p className={styles.habitMeta}>
              {isDaily 
                ? (completedCount > 0 ? 'Completed for today!' : 'Pending for today')
                : `This Week: ${completedCount} / ${habit.targetCount}`
              }
            </p>
          </div>
      ...
      <Modal
        ...
        actions={
          <>
            ...
            <button className={`${modalStyles.actionButton} ${modalStyles.confirmButton}`} onClick={handleDelete}>Delete</button>
          </>
        }
      >
        <p>Are you sure you want to delete the habit "<strong>{habit.name}</strong>"? This action cannot be undone.</p>
      </Modal>
    </>
  );
}
