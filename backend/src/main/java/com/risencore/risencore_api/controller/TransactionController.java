...
@Tag(
        ...
        description = "Endpoints for managing user's financial transactions")
public class TransactionController {
    ...
    @Operation(
            ...
            description =
                    "Retrieves a list of all financial transactions for the currently authenticated user, ordered by date descending.",
    ...
    @Operation(
            ...
            description =
                    "Creates a new financial transaction (income or expense) for the currently authenticated user.",
            security = @SecurityRequirement(name = "bearerAuth"))
    @PostMapping
    public ResponseEntity<TransactionDTO> createTransactionForCurrentUser(
            @Valid @RequestBody CreateTransactionDTO createDto) {
        TransactionDTO newTransaction =
                transactionService.createTransactionForCurrentUser(createDto);
    ...
    @Operation(
            ...
            description =
                    "Retrieves a summary of expenses grouped by category for the currently authenticated user.",
            security = @SecurityRequirement(name = "bearerAuth"))
    @GetMapping("/summary/by-category")
    public ResponseEntity<List<CategoryExpenseDTO>> getExpenseSummaryByCategory() {
        List<CategoryExpenseDTO> summary =
                transactionService.getExpenseSummaryByCategoryForCurrentUser();
    ...
    @Operation(
            ...
            description =
                    "Deletes a specific transaction by its ID. A user can only delete their own transactions.",
            security = @SecurityRequirement(name = "bearerAuth"))
    @DeleteMapping("/{id}")
