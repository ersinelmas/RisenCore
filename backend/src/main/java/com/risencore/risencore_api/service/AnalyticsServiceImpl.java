...
public class AnalyticsServiceImpl implements AnalyticsService {
    ...
    public String generateWeeklyReview() {
        ...
        Optional<WeeklyReview> existingReview =
                weeklyReviewRepository.findByUserAndWeekStartDate(
                        currentUser, currentReviewWeekStart);
        ...
        List<Transaction> recentTransactions =
                transactionRepository.findByUserAndTransactionDateAfter(
                        currentUser, weekStart.toLocalDate());
    ...
    private LocalDateTime calculateReviewWeekStart(LocalDateTime now) {
        ...
        if (now.getDayOfWeek() == DayOfWeek.SUNDAY && now.getHour() < 19) {
            lastSunday = lastSunday.minusWeeks(1);
                ...
                && now.isBefore(lastSunday.withHour(19))) {
    ...
    private String buildWeeklyReviewPrompt(
            User user,
            List<Task> tasks,
        ...
        sb.append(
                "You are RisenCore, a friendly, insightful, and motivating digital life assistant. ");
        ...
        sb.append("Your task is to analyze the following JSON data for the user '")
                .append(user.getFirstName())
                .append("' ");
        sb.append("for the period ").append(start).append(" to ").append(end).append(". ");
        sb.append(
                "Generate a short, encouraging weekly review consisting of 3 to 4 bullet points. ");
        sb.append(
                "Focus on finding at least one interesting pattern or a significant achievement. ");
        sb.append(
                "Use Markdown for formatting (e.g., use '*' for bullet points and '**' for bold text).\n\n");
        ...
        String habitsJson =
                habits.stream()
                        .map(
                                h -> {
                                    long completionsLastWeek =
                                            h.getCompletions().stream()
                                                    .filter(
                                                            c ->
                                                                    c.getCompletionDate()
                                                                            .isAfter(start))
                                    ...
                                    return String.format(
                                            "{\"name\": \"%s\", \"completions_last_week\": %d}",
